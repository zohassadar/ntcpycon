...
class WebSocketPayload:
    def __init__(self, payload: bytes):
        ...
        logger.debug(f'First byte has int value of {first_byte}.  Remaining length is {len(blob)}')
        ...
        logger.debug(f'Second byte has int value of {second_byte}.  Remaining length is {len(blob)}')
        ...
        if self.mask:
            ...
            logger.debug(f'Mask is {self.key!r}.  Remaining length of data is {len(blob)}')
        else:
            self.data = blob
            return
    ...
    @classmethod
    def load_packetlist(cls, packetlist: PacketList) -> list[WebSocketPayload]:
...
def filter_packets_with_raw(packets: PacketList) -> PacketList:
    packets_with_payload = [p for p in packets if isinstance(p.payload.payload.payload, Raw)]
    logger.debug(f'Filtered {len(packets)} -> {len(packets_with_payload)} - Raw payload')
    return packets_with_payload

def extract_payloads_dict(packets: PacketList) -> dict[int, bytes]:
    payloads = {p.payload.fields.get('id'): p.payload.payload.payload.load for p in packets}
    logger.debug(f'Extracted {len(payloads)}')
    return payloads

def extract_payloads(packets: PacketList) -> list[bytes]:
    payloads = [p.payload.payload.payload.load for p in packets]
    logger.debug(f'Extracted {len(payloads)}')
    return payloads


packets = read_pcap_file('example.pcap')
from_client = filter_packets_by_dest(packets, '192.168.42.42')
